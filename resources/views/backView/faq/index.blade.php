...
@section("titleBack","FAQ Index")
@section("contentBack")
    <div class="col-12 my-5 text-center mx-auto">
        <h1 class="text-white text-center text-capitalize mb-3">FAQ details</h1>
        <div class="mb-5 bg-white p-3 rounded-3">
            <table class="table table-dark table-responsive text-white rounded-3 px-2 py-4">
                <tr>
                    ...
                    <td class="text-center text-capitalize">questionFa</td>
                    <td class="text-center text-capitalize">questionEn</td>
                    <td class="text-center text-capitalize">answerFa</td>
                    <td class="text-center text-capitalize">answerEn</td>
                ...
                @foreach($faq as $item)
                    <tr>
                        ...
                        <td style="line-height: 120px">
                            <img src="{{asset("images/faq/".$item->image)}}" alt="">
                        </td>
                        <td style="line-height: 120px">{{mb_strcut($item->altFa,0,10)}}</td>
                        <td style="line-height: 120px">{{$item->altEn}}</td>
                        <td style="line-height: 120px;">{{mb_strcut($item->questionFa,0,15)}}</td>
                        <td style="line-height: 120px">{{substr($item->questionEn,0,20)}}</td>
                        <td style="line-height: 120px">{{mb_strcut($item->answerFa,0,15)}}</td>
                        <td style="line-height: 120px">{{substr($item->answerEn,0,20)}}</td>
                        ...
                        <td style="line-height: 120px">
                            {!! Form::open(["route"=>["faq.destroy","id"=>$item->id],"method"=>"delete"]) !!}
                            {!! Form::submit("delete",["class"=>"btn btn-danger text-capitalize text-white"]) !!}
                            {!! Form::close() !!}
                        </td>
                        <td style="line-height: 120px">
                            <a class="btn btn-warning text-capitalize text-white" href="{{route("faq.edit",["id"=>$item->id])}}">update</a>
                        </td>
                    </tr>
                @endforeach
            </table>
        ...
        @endif
        <a class="btn btn-success text-white text-capitalize mt-5" href="{{route("faq.create")}}">create FAQ</a>
    </div>
@endsection
