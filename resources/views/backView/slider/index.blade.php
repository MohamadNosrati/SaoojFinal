...
<div class="col-12 my-5 text-center mx-auto">
            ...
            @foreach($slider as $item)
                <tr>
                    ...
                    <td style="line-height: 120px">
                        <img style="width:125px;height: 125px;border-radius: 50%;object-fit: cover " src="{{asset("images/slider/".$item->image)}}">
                    </td>
                    <td style="line-height: 120px">{{mb_strcut($item->altFa,0,10)}}</td>
                    <td style="line-height: 120px">{{$item->altEn}}</td>
                    <td style="line-height: 120px">{{mb_strcut($item->titleFa,0,10)}}</td>
                    ...
                    <td style="line-height: 120px">
                        <a class="btn btn-warning text-capitalize text-white" href="{{route("slider.edit",["id"=>$item->id])}}">update</a>
                    </td>
                </tr>
            @endforeach
        </table>
    ...
    @endif
    <a class="btn btn-success text-white text-capitalize mt-5" href="{{route("slider.create")}}">create slider</a>
</div>
@endsection
