...
@section("titleBack","Blog Index")
@section("contentBack")
    <div class="col-12 my-5 text-center mx-auto">
        <h1 class="text-white text-center text-capitalize mb-3">blog details</h1>
                ...
                @foreach($blog as $item)
                    <tr>
                        ...
                        <td style="line-height: 120px">
                            <img style="width:125px;height: 125px;border-radius: 50%;object-fit: cover " src="{{asset("images/blog/".$item->image)}}">
                        </td>
                        <td style="line-height: 120px">{{mb_strcut($item->altFa,0,10)}}</td>
                        <td style="line-height: 120px">{{$item->altEn}}</td>
                        <td style="line-height: 120px">{{mb_strcut($item->textFa,0,20)}}</td>
                        ...
                        <td style="line-height: 120px">
                            {!! Form::open(["route"=>["blog.destroy","id"=>$item->id],"method"=>"delete"]) !!}
                            {!! Form::submit("delete",["class"=>"btn btn-danger text-capitalize text-white"]) !!}
                            {!! Form::close() !!}
                        </td>
                        <td style="line-height: 120px">
                            <a class="btn btn-warning text-capitalize text-white" href="{{route("blog.edit",["id"=>$item->id])}}">update</a>
                        </td>
                    </tr>
                @endforeach
            </table>
        ...
        @endif
        <a class="btn btn-success text-white text-capitalize mt-5" href="{{route("blog.create")}}">create blog</a>
    </div>
@endsection
