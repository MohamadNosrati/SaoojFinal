...
@section("titleBack","English Comment Index")
@section("contentBack")
    <div class="col-12 my-5 text-center mx-auto">
        <h1 class="text-white text-center text-capitalize mb-3">English Comment details</h1>
            ...
            <table class="table table-dark table-responsive text-white rounded-3 px-2 py-4">
                ...
                @foreach($comment as $item)
                    <tr>
                        ...
                        <td>
                            {!! Form::open(["route"=>["commentEn.destroy","id"=>$item->id],"method"=>"delete"]) !!}
                            {!! Form::submit("delete",["class"=>"btn btn-danger text-capitalize text-white"]) !!}
                            {!! Form::close() !!}
                        </td>
                        <td>
                            <a class="btn btn-warning text-capitalize text-white" href="{{route("commentEn.edit",["id"=>$item->id])}}">update</a>
                        </td>
                    </tr>
                @endforeach
            </table>
