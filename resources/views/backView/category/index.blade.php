...
@section("titleBack","Category Index")
@section("contentBack")
    <div class="col-12 my-5 text-center mx-auto">
        <h1 class="text-white text-center text-capitalize mb-3">Category details</h1>
                ...
                @foreach($category as $item)
                    <tr>
                        ...
                        <td style="line-height: 120px">
                            <img style="width:125px;height: 125px;border-radius: 50%;object-fit: cover " src="{{asset("images/category/".$item->image)}}" alt="">
                        </td>
                        <td style="line-height: 120px">{{mb_strcut($item->altFa,0,10)}}</td>
                        <td style="line-height: 120px">{{$item->altEn}}</td>
                        <td style="line-height: 120px">{{substr($item->nameFa,0,20)}}</td>
                        ...
                        <td style="line-height: 120px">
                            <a class="btn btn-warning text-capitalize text-white" href="{{route("category.edit",["id"=>$item->id])}}">update</a>
                        </td>
                    </tr>
                @endforeach
            </table>
        ...
        @endif
        <a class="btn btn-success text-white text-capitalize mt-5" href="{{route("category.create")}}">create categroy</a>
    </div>
@endsection
